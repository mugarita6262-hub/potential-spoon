...
| ダブルクリック | 中身 |
|---|---|
| **nightly.bat** | prepare（候補選定＋キャプション生成）→ post（新規10件をタブで開く） |
| **post_now.bat** | post のみ |
| **prune.bat** | 古い投稿をまとめて削除（`--max 100`）。空きは自分で確保しておく |
| **prune_preview.bat** | 削除せずに対象を確認 |
| **post_test.bat** | 投稿せずに1件ずつ流し込みを確認（ドライラン） |

> 投稿は登録数を増やすので、`prune.bat` を定期的に回して 20,000件の上限に余裕を持たせておくこと。
